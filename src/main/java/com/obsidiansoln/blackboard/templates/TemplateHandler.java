...
public class TemplateHandler implements RestHandler {
	...
	public HTTPStatus readObject(String host, String access_token, RequestData data) {
		...
		if (data != null && data.getTemplateId() != null) {
			return (RestRequest.sendRequest(
					host, RestConstants.COURSE_PATH + RestConstants.COURSE_COURSEID_QUERY + data.getTemplateId(),
	...
	public HTTPStatus updateObject(String host, String access_token, RequestData data, TemplateProxy proxy) {
	...
	public TemplateResponseProxy getClientData(String host, String access_token, String p_nextPage, RequestData p_data) {
		...
		TemplateResponseProxy obj = null;

		if (data != null) {
			// JSON file to Java object
			try {
				obj = mapper.readValue(data, TemplateResponseProxy.class);
	...
	public TemplateProxy getClientData2(String host, String access_token, String p_nextPage, RequestData p_data) {
		...
		TemplateProxy obj = null;

		if (data != null) {
			// JSON file to Java object
			try {
				obj = mapper.readValue(data, TemplateProxy.class);
			} catch (JsonParseException e) {
				log.error("Json Parser Error: ", e);
			} catch (JsonMappingException e) {
				log.error("Json Mapping Error: ", e);
	...
	private String getBody(TemplateProxy p_proxy) {
		log.trace("In getBody()");
		ObjectMapper objMapper = new ObjectMapper();
