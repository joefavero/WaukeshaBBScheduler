...
public class ColumnHandler implements RestHandler {

	private static final Logger log = LoggerFactory.getLogger(ColumnHandler.class);
	...
	public HTTPStatus readObject(String host, String access_token, RequestData data) {
		...
		if (data.getColumnId() != null) {
			return (RestRequest.sendRequest(host, RestConstants.GRADEBOOK_PATH + data.getCourseId()
					+ RestConstants.GRADEBOOK_COLUMNS + data.getColumnId(), HttpMethod.GET, access_token, ""));
		} else {
	...
	public ColumnResponseProxy getClientData(String host, String access_token, String p_nextPage, RequestData p_data) {
		...
		ColumnResponseProxy obj = null;

		if (data != null) {
			// JSON file to Java object
			try {
				obj = mapper.readValue(data, ColumnResponseProxy.class);
	...
	public ColumnProxy getClientData2(String host, String access_token, String p_nextPage, RequestData p_data) {
		log.trace("In getClientData()");
		String data = null;
		if (p_nextPage == null) {
			data = readObject(host, access_token, p_data).getData();
		...
		ColumnProxy obj = null;

		if (data != null) {
			// JSON file to Java object
			try {
				obj = mapper.readValue(data, ColumnProxy.class);
			} catch (JsonParseException e) {
				log.error("Json Parser Error: ", e);
			} catch (JsonMappingException e) {
				log.error("Json Mapping Error: ", e);
