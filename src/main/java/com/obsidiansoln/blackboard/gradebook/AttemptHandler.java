...
public class AttemptHandler implements RestHandler {
	...
	public HTTPStatus readObject(String host, String access_token, RequestData data) {
		...
		if (data.getColumnId() != null) {
			if (data.getUserId() != null) {
				if (data.getAttemptDate() != null) {
					return (RestRequest.sendRequest(host,
							RestConstants.GRADEBOOK_PATH + data.getCourseId() + RestConstants.GRADEBOOK_COLUMNS
									+ data.getColumnId() + RestConstants.GRADEBOOK_ATTEMPTS
									+ RestConstants.GRADEBOOK_ATTEMPTS_STATUS_PARAMETER + "&userId=" + data.getUserId()
									+ RestConstants.GRADEBOOK_ATTEMPT_DATE_COMPARE_PARAMETER + data.getAttemptDate(),
				...
				if (data.getAttemptDate() != null) {
					return (RestRequest.sendRequest(host,
							RestConstants.GRADEBOOK_PATH + data.getCourseId() + RestConstants.GRADEBOOK_COLUMNS
									+ data.getColumnId() + RestConstants.GRADEBOOK_ATTEMPTS
									+ RestConstants.GRADEBOOK_ATTEMPTS_STATUS_PARAMETER
									+ RestConstants.GRADEBOOK_ATTEMPT_DATE_COMPARE_PARAMETER + data.getAttemptDate(),
							HttpMethod.GET, access_token, ""));
				} else {
					return (RestRequest.sendRequest(host,
	...
	public AttemptResponseProxy getClientData(String host, String access_token, String p_nextPage, RequestData p_data) {
		...
		AttemptResponseProxy obj = null;

		if (data != null) {
			// JSON file to Java object
			try {
				obj = mapper.readValue(data, AttemptResponseProxy.class);
	...
	public AttemptProxy getClientData2(String host, String access_token, String p_nextPage, RequestData p_data) {
		log.trace("In getClientData()");
		String data = null;
		if (p_nextPage == null) {
			data = readObject(host, access_token, p_data).getData();
		...
		AttemptProxy obj = null;

		if (data != null) {
			// JSON file to Java object
			try {
				obj = mapper.readValue(data, AttemptProxy.class);
			} catch (JsonParseException e) {
				log.error("Json Parser Error: ", e);
			} catch (JsonMappingException e) {
				log.error("Json Mapping Error: ", e);
